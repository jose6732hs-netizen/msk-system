...
import { Copy, Eye, Gift, KeyRound, Loader2, Plus, Timer, Clock } from "lucide-react";
...
function useServerNow(serverTime) {
...
function remaining(now, expiresAt, activatedAt) {
    if (!expiresAt)
        return null;
    const targetTime = new Date(expiresAt).getTime();
    const diff = targetTime - now;
    if (diff <= 0)
        return { expired: true, label: "EXPIRADO", progress: 0, time: { d: 0, h: 0, m: 0, s: 0 } };
...
function TimeDisplay({ value, label, danger }) {
    return (<div className="flex min-w-[3rem] flex-col items-center rounded-xl border border-white/5 bg-black/40 p-2 shadow-inner">
...
function Metric({ label, value }) {
    return (<div className="rounded-xl border border-border/60 bg-muted/20 p-4">
...
export function TokenManager() {
    ...
    async function onGenerate() {
        ...
        try {
            ...
            const detail = tokens.find((t) => t.id === res.licenseId);
    ...
    return (<section className="glass rounded-2xl p-7 lg:col-span-2">
      <Tabs defaultValue="tokens">
        ...
        <TabsContent value="tokens" className="mt-6">
          {isLoading ? (<div className="flex justify-center py-10">
              ...
              <div className="grid gap-3 sm:grid-cols-4">
                ...
                <Metric label="Renovação" value={allowance?.renewal
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                ...
                {available <= 0 && (<p className="text-sm text-destructive">
                    Nenhum token ativo ainda! Navegue pelo site e garanta o seu acesso premium.
                  </p>)}
              </div>

              {fresh && (<div className="mt-5 rounded-xl border border-primary/25 bg-primary/5 p-4">
              ...
              <div className="mt-7 grid gap-3 md:grid-cols-2">
                {tokens.length === 0 ? (<p className="py-6 text-center text-sm text-muted-foreground md:col-span-2">
                    Nenhum token gerado ainda.
                  </p>) : (tokens.map((t) => {
                const left = remaining(now, t.expires_at, t.activated_at || t.created_at);
                return (<div key={t.id} className="group relative flex flex-col rounded-[2rem] border border-white/10 bg-[#0F0F0F] p-6 transition-all duration-500 hover:border-primary/50 hover:shadow-[0_0_40px_-10px_rgba(var(--primary-rgb),0.2)]">
                        <div className="flex items-center justify-between mb-6">
                          <h4 className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-muted-foreground">Sua licença</h4>
                          <span className={`rounded-full border px-3 py-1 text-[0.6rem] font-black uppercase tracking-widest ${statusStyle(t.status)}`}>
                        ...
                        <div className="relative mb-6 rounded-2xl border border-white/5 bg-black/40 p-5 group-hover:border-primary/20 transition-colors">
                          ...
                          {t.status === 'revoked' ? (<div className="py-4 text-center">
                              ...
                              <div className="flex gap-2">
                                <Button size="sm" variant="ghost" className="h-9 px-4 rounded-xl border border-white/5 bg-white/5 text-[0.6rem] font-black uppercase tracking-widest hover:bg-white/10" onClick={() => onReveal(t.id)}>
                                  <Eye className="h-3.5 w-3.5 mr-2"/> Revelar
                                </Button>
                                <Button size="sm" variant="neon" className="h-9 px-4 rounded-xl text-[0.6rem] font-black uppercase tracking-widest shadow-lg shadow-primary/10" onClick={() => copy(t.preview)}>
                          ...
                          {left ? (<div className="space-y-3">
                              {t.status === 'active' ? (<>
                                  ...
                                  <div className="relative h-2.5 w-full overflow-hidden rounded-full border border-primary/20 bg-black/40">
                                    <div className="h-full bg-gradient-to-r from-primary/50 to-primary animate-pulse shadow-[0_0_15px_rgba(var(--primary-rgb),0.6)]" style={{ width: '100%' }}/>
                                  </div>
                                  ...
                                  <div className="relative h-2.5 w-full overflow-hidden rounded-full border border-white/5 bg-black/40">
                                    <div className={`h-full transition-all duration-1000 ease-linear ${left.expired ? "bg-destructive" : "bg-gradient-to-r from-primary/50 to-primary"}`} style={{ width: `${left.expired ? 100 : left.progress}%` }}/>
                                  </div>
                            ...
                            </div>) : (<span className="inline-block rounded-full border border-white/5 bg-white/5 px-3 py-1 text-[0.65rem] font-bold uppercase tracking-widest text-muted-foreground">
                              Vitalício
                            </span>)}
                        </div>
        ...
        <TabsContent value="trial" className="mt-6">
          <div className="rounded-[2rem] border border-primary/20 bg-[#0F0F0F] p-8 shadow-2xl relative overflow-hidden group">
            ...
            {trial?.state === "running" ? (<div className="space-y-6">
                ...
                return (<div className="space-y-4">
                        ...
                        <div className="relative h-4 w-full bg-black/40 rounded-full overflow-hidden border border-white/10 shadow-inner">
                          <div className="h-full bg-gradient-to-r from-primary/40 to-primary transition-all duration-1000 ease-linear shadow-[0_0_20px_rgba(var(--primary),0.8)]" style={{ width: `${progress}%` }}/>
                        ...
                        {fresh?.token ? (<div className="rounded-2xl border border-white/5 bg-black/40 p-5 mt-4">
                          ...
                          </div>) : (<p className="text-xs text-muted-foreground text-center">Token ativo na sua conta. Use na extensão para testar.</p>)}
                      </div>);
            })()}
                </div>
              </div>) : trial?.state === "used" ? (<div className="space-y-6">
                <div className="flex items-center gap-3">
                ...
                const sTotal = Math.max(0, Math.floor(diff / 1000));
                if (sTotal <= 0)
                    return <p className="text-xl font-black text-primary uppercase">Disponível agora!</p>;
              ...
              </div>) : (<div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                <Button variant="neon" className="w-full sm:w-auto h-14 px-10 text-[0.7rem] font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-primary/20" onClick={onTrial} disabled={busy}>
