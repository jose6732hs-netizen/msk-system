...
Deno.serve(async req => {
  ...
  try {
    if (req.method === "POST" && url.searchParams.get("action") === "health") {
      return json({ ok: true, service: "msk-agent", auth: "chat-open-edit-session", version: "1.9.0" });
    }
    ...
    // Conversa comum não depende da sessão de escrita do GitHub. A sessão MSK
    // continua obrigatória somente para status/tarefas/aprovação/edições reais.
    if (action === "chat") {
      ...
      const historyText = history.map((item: any) => {
        const role = item?.role === "assistant" ? "MSK" : "Cliente";
        const content = String(item?.content || item?.message || "").slice(0, 8000);
        return content ? `${role}: ${content}` : "";
      }).filter(Boolean).join("\n");
      const response = await openai({
        model: "gpt-5.6-sol",
        max_output_tokens: 3000,
        input: `Você é o MSK Agente dentro da extensão do Lovable. Responda em português do Brasil, de forma objetiva e útil. O chat deve continuar funcionando mesmo quando a autorização de escrita do GitHub ainda não estiver disponível. Não diga que uma alteração foi aplicada se você apenas respondeu em conversa. Projeto Lovable atual: ${projectId}. Nome: ${String(body.project_name || "não informado").slice(0, 200)}.\n\nHistórico recente:\n${historyText || "(sem histórico)"}\n\nCliente: ${message}\nMSK:`
      });
      const messageText = outputText(response).trim();
      return json({ ok: true, connected: true, mode: "chat", message: messageText || "Recebi sua mensagem. Como posso ajudar neste projeto?", response_id: response.id });
    }

    const session = req.headers.get("x-msk-session") || "";
    if (!await validSession(projectId, session)) return json({ error: "Sessão de edição MSK ainda não autorizada.", code: "MSK_SESSION_REQUIRED", connected: false }, 401);
