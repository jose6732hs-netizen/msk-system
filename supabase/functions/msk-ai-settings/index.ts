...
async function encryptionKey() {
  return crypto.subtle.importKey("raw", await encryptionMaterial(), "AES-GCM", false, ["encrypt"]);
}
...
async function currentAdmin(req: Request) {
  ...
  const { data: roles, error: rolesError } = await saas
    .from("user_roles").select("role").eq("user_id", data.user.id).in("role", ["admin", "super_admin"]);
...
async function validateBaiKey(apiKey: string) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(String(body?.error?.message || body?.message || `A IA respondeu HTTP ${response.status}`).slice(0, 300));
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "ai-global-status") {
      const { data, error } = await db.from("msk_ai_settings")
