...
const hmac = async (v: string) => { const k = await crypto.subtle.importKey("raw", enc.encode(env("MSK_STATE_SECRET")), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]); return b64(new Uint8Array(await crypto.subtle.sign("HMAC", k, enc.encode(v)))); };
export const makeState = async (projectId: string, returnUrl: string, repository: string, userId: string) => { const p = b64(enc.encode(JSON.stringify({ projectId, returnUrl, repository, userId, exp: Date.now() + 900000 }))); return `${p}.${await hmac(p)}`; };
export const readState = async (s: string) => { const [p, q] = s.split("."); if (!p || !q || q !== await hmac(p)) throw new Error("STATE_INVALID"); const d = JSON.parse(dec.decode(unb64(p))); if (Number(d?.exp || 0) < Date.now() || !/^https:\/\/lovable\.dev\/projects\//.test(String(d?.returnUrl || ""))) throw new Error("STATE_EXPIRED"); return d; };
