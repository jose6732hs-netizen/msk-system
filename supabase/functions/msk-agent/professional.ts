...
export const repairPrompt = (command: string, repository: string, previous: string) => `${MSK_ENGINEERING_PROFILE}${skillContext(command)}\nSua saída anterior não pôde ser aplicada com segurança. Refaça como JSON estritamente válido, sem markdown, obedecendo ao pedido/contexto e ao repositório ${repository}. Não omita conteúdo de arquivos.\nPedido/contexto: ${command}\nSaída anterior para corrigir:\n${previous.slice(0, 18000)}`;
