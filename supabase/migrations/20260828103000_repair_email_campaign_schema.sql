-- Repair for environments where the email broadcast migrations were not applied.
-- Idempotent by design so it is safe on databases where the tables already exist.

create table if not exists public.email_campaigns (
  id uuid primary key default gen_random_uuid(),
  campaign_key text not null unique,
  subject text not null,
  new_whatsapp text not null default '',
  from_email text,
