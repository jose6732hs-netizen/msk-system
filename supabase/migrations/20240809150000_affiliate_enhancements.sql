...
-- 2. Ensure RLS for affiliate tables
...
-- 4. Admin Policies (Super Admin)
-- Assuming is_admin(auth.uid()) helper exists or public.has_role(auth.uid(), 'admin')
...
-- 5. Seed default settings if not present
...
-- 6. Add unique constraint for referral code registration to avoid duplicates
-- (Table affiliates already has UNIQUE on code column based on previous \d output)
